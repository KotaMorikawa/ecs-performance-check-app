    ...
    const collectMetrics = async () => {
      ...
      try {
        ...
        const renderMetrics: RenderMetrics = {
          ...
          clientRenderTime: performance.now(),
          hydrationTime: navigationTiming.domContentLoadedEventEnd - navigationTiming.domContentLoadedEventStart,
        ...
        const networkMetrics: NetworkMetrics = {
          ...
          avgResponseTime: resourceTiming.length > 0 
            ? resourceTiming.reduce((acc, entry) => acc + (entry.responseEnd - entry.requestStart), 0) / resourceTiming.length
          ...
          errors: resourceTiming.filter(entry => entry.transferSize === 0 && entry.decodedBodySize === 0).length
        };
        ...
        const cacheMetrics: CacheMetrics = {
          ...
          browserCacheHits: resourceTiming.filter(entry => entry.transferSize === 0 && entry.decodedBodySize > 0).length,
          browserCacheMisses: resourceTiming.filter(entry => entry.transferSize > 0).length,
          cacheEfficiency: 0
        };
        
        cacheMetrics.cacheEfficiency = 
          cacheMetrics.nextjsCacheHits / (cacheMetrics.nextjsCacheHits + cacheMetrics.nextjsCacheMisses) * 100;
    ...
    setTimeout(() => {
      ...
      setMetrics(prev => prev ? {
        ...
        network: {
          ...
          avgResponseTime: Math.max(50, prev.network.avgResponseTime + (Math.random() - 0.5) * 20),
          cacheHitRate: Math.min(100, Math.max(0, prev.network.cacheHitRate + (Math.random() - 0.5) * 10))
        }
      } : null);
      setIsCollecting(false);
    }, 1000);
...
function calculateCacheHitRate(resourceTiming: PerformanceResourceTiming[]): number {
  ...
  const cacheHits = resourceTiming.filter(entry => 
