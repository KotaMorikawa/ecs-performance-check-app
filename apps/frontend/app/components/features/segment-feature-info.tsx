...
const segmentFeatures = {
  routing: {
    basic: {
      ...
      description:
        "Next.js 15.3.4 の App Router を使用した基本的なルーティング機能のデモンストレーション",
      capabilities: [
        "ファイルベースルーティング",
        "Server Components",
        "Client Components",
    ...
    dynamic: {
      ...
      technicalDetails: ["TypeScript型安全なパラメータ処理", "Static Generation at Build Time"],
    },
    nested: {
      title: "ネストされたレイアウト",
  ...
  "server-actions": {
    basic: {
      ...
      technicalDetails: ["use server ディレクティブ", "Next.js revalidatePath API"],
    },
  },
  "data-fetching": {
    basic: {
      ...
      technicalDetails: ["fetch with revalidate options", "generateStaticParams"],
    },
  },
  caching: {
    basic: {
      ...
      capabilities: ["Request Memoization", "Data Cache", "Full Route Cache", "Router Cache"],
      verificationSteps: [
        {
          step: "キャッシュ確認",
      ...
      technicalDetails: ["fetch caching strategies", "Cache Tags & Revalidation"],
    },
  },
  streaming: {
    basic: {
      ...
      technicalDetails: ["Suspense boundaries", "Streaming Response"],
    },
  },
  middleware: {
    basic: {
      ...
      capabilities: ["Authentication", "Redirects", "Request/Response modification", "A/B Testing"],
      verificationSteps: [
        {
          step: "Middleware動作確認",
      ...
      technicalDetails: ["Edge Runtime", "NextRequest/NextResponse"],
    },
  },
  "image-optimization": {
    basic: {
      ...
      technicalDetails: ["Image Optimization API", "Sharp image processing"],
    },
  },
  metadata: {
    basic: {
      ...
      technicalDetails: ["Metadata API", "generateMetadata function"],
    },
  },
};
...
export function SegmentFeatureInfo({ segmentType, subType = "basic" }: SegmentFeatureInfoProps) {
  ...
  const featureData = featureGroup
    ? (featureGroup as Record<string, FeatureDataStructure>)[subType]
    : undefined;

  if (!featureData) {
    return (
      <div className="bg-gray-100 border border-gray-300 rounded-lg p-6 mb-8">
        <h2 className="text-xl font-semibold text-gray-700 mb-2">機能実装予定</h2>
