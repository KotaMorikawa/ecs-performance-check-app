...
import { getMetricStatus, metricDescriptions, useWebVitals } from "@/hooks/use-web-vitals";
...
function MetricCard({ name, value, unit }: MetricCardProps) {
  ...
  return (
    <div className={cn("rounded-lg border p-4 transition-all duration-200", statusColors[status])}>
      <div className="flex items-start justify-between mb-2">
        <h4 className="font-semibold text-sm uppercase">{name.toUpperCase()}</h4>
